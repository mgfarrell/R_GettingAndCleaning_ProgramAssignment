...
# Steps to load source data into R
...
# Combines X Train and X test data into single data set and renames columns
...
# Combines Subject Train and Test data into a single data set, renames column, binds column with YX_Train_Test, and merges Actvity into data set.
...
# Clean up column names in R before selecting mean & standard deviation columns
...
# Select columns for tidy data set.  Create vector to identify mean and std deviation columns.  Use vector to select columns from 
...
# Renaming of columns with slightly more readable names
...
# Steps to groups for Subject & Activity variables and calculate mean for each 'mean' and 'std dev' variable
